...
export async function processVideo(
  ...
  try {
    ...
    const cuts = await detectSceneCuts(originalPath);
    const candidates = buildCandidateSegments(meta.durationMs, cuts);

    await prisma.$transaction([
      prisma.videoSegment.deleteMany({ where: { videoId } }),
      prisma.videoSegment.createMany({
    ...
    await aiQueue.add(
      "analyze",
      { videoId, sessionId: video.sessionId },
      { jobId: videoId, removeOnComplete: 100, removeOnFail: 100 }
    );

    console.log(
      `[video-worker] ${videoId} ready：${candidates.length} 个候选片段，${frameFiles.length} 帧`
    );
  } finally {
